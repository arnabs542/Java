...
public class Car_Pooling {
	...
	public boolean carPooling(int[][] trips, int capacity) {
		...
		for (int[] i: trips) {
			laststop = Math.max(laststop, i[2] );
			
			int prevBoard = map.getOrDefault(i[1], 0);
			map.put(i[1], prevBoard + i[0] );
			
			int prevDrop = map.getOrDefault(i[2], 0);
			map.put(i[2], prevBoard - i[0] );
		}
		
		int currPassenger = 0;
		
		for (int i = 0; i <= laststop; i ++ ) {
			int val = map.getOrDefault(i, Integer.MIN_VALUE);
